...
## Version 1.3 materials and geometry

`../art/alpine_limestone.png` is an original texture generated by Codex with the built-in imagegen tool on September 14, 2026 (1254 × 1254). It replaces `rock.png` on the quarry cliff faces, distant crags and sculpted cat entrance. It uses mipmaps and world-space triplanar projection; the earlier rock texture remains preserved in this folder.

Generation brief supplied with the new texture: seamless, flat-lit neutral alpine limestone with broad vertical fissures and slab fractures, subtle fine grain, warm chalk tones and sparse moss in crevices. No baked directional shadows. No external stock textures or third-party game assets were used.

The 1.3 scenery is actual 3D: fractured cliff volumes, irregular boulders, a bevelled masonry arch, a stone cat relief, suspended timber walkway, cupped pine bough meshes with needle clusters, grass and petalled flowers, flowing curved cascade meshes and a reflective animated lake. FastNoiseLite generates additional normal-map detail for dirt and rock and a cached 1024-square cloud texture. All animated world materials use the shared reduced-motion clock. Forward+ Metal adds restrained ambient occlusion, ACES tone mapping, soft sunlight shadows and distance haze. SSIL and volumetric fog are disabled in the final balanced settings. Forward+-specific effects are guarded when using the Compatibility fallback.
