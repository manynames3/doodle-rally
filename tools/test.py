...
for suite in ("test_race.gd", "test_menu.gd", "test_roster_stage.gd", "test_core_assets.gd", "test_integration.gd", "test_world.gd", "test_audio.gd"):
...
for label, extra in (("straight", []), ("quarry", ["--course=1"]), ("minecraft", ["--course=1", "--mode=minecraft"])):
    ...
    if result.returncode or "ERROR:" in output:
        raise SystemExit(result.returncode or 1)
print("All seven functional suites and three temporal collision replays passed.")
