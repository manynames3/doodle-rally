...
**Doodle Rally — Cat Racers 1.3.5** fixes Character Select so selecting Zizi no longer swaps the supplied art for the older procedural model. The focused racer now spins the exact same transparent `select_sprite.png` used on its card; the other seven cards remain unchanged, and the selected card image is hidden to avoid duplicates. The art rotates continuously through one full turn every six seconds. The pack contains one view per cat, so its cutout mirrors during the back half rather than showing a newly modeled rear view. Reduced Motion holds the preview still.

The Universal app is `builds/Doodle Rally 1.3.5.app`, packaged as `Doodle_Rally_1.3.5_Mac.zip` beside this repository. **574 functional checks** passed across seven test suites, along with six course/mode world builds and three temporal collision replays. The packaged app passed **27 native launch, menu, Garage, race, drive, and results cases**, including front, quarter-, half-, and three-quarter-turn character previews. Verification logs and screenshots are in `docs/test-results/1.3.5/` and `screenshots/1.3.5/`. The app is locally signed, not Apple-notarized.
