...
**Doodle Rally — Cat Racers 1.3.4** simplifies Character Select to one racer image per card. The selected cat and kart replace their cutout with the existing 3D model and rotate continuously through one full turn every six seconds; seven unselected racers keep their high-resolution supplied cutouts. Changing racer selection moves the turntable to the newly focused cat. The rotation uses constant angular speed through its loop boundary, and Reduced Motion holds the racer at a fixed angle.

The Universal app is `builds/Doodle Rally 1.3.4.app`, packaged as `Doodle_Rally_1.3.4_Mac.zip` beside this repository. **571 functional checks** passed across seven test suites, along with six course/mode world builds and three temporal collision replays. The packaged app passed **27 native launch, menu, Garage, race, drive, and results cases**, including front, quarter-, half-, and three-quarter-turn character previews. Verification logs and screenshots are in `docs/test-results/1.3.4/` and `screenshots/1.3.4/`. The app is locally signed, not Apple-notarized.
