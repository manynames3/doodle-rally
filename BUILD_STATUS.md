...
The Desktop Dojo video exposed one remaining flicker source: the paper sheet and wooden deck shared an exactly coplanar top surface. The deck top is now at y=2.8 and the sheet top at y=3.0; the existing removal of the shallow-angle graph-paper strips remains in place. The project regression checks this 0.2 m clearance. The native 12-second Desktop Dojo drive completed and reported 8.465 ms median / 9.662 ms p95 frame intervals; the fresh capture is `screenshots/1.3.6/drive_desktop_dojo.png`.

The playable Universal app is `builds/Doodle Rally 1.3.6.app`; its ZIP is `Doodle_Rally_1.3.6_Mac.zip` beside this repository. The app uses the existing Godot 4.7.2 Universal runtime with the newly exported 1.3.6 project pack, updated bundle version, and local ad-hoc signature. It passed **34 native packaged-app cases**, including all eight selected-cat screens, three spin phases, all courses, and four 12-second drives. The seven source suites, six course/mode world builds, and three temporal collision replays passed. Verification logs and screenshots are in `docs/test-results/1.3.6/` and `screenshots/1.3.6/`. The app is locally signed, not Apple-notarized.
